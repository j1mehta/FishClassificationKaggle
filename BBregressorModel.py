...
#The valid means there is no padding around input or feature map, while 
#same means there are some padding around input or feature map, making 
#the output feature map's size same as the input's

#Check out model.output_shape at every level for sanity

conv1 = Convolution2D(32,9,9,border_mode='valid', W_regularizer=l1(wr), init='lecun_uniform')
conv_l1 = conv1(frame_in)
Econv_l1 = ELU()(conv_l1)
pool_l1 = MaxPooling2D(pool_size=(2,2))(Econv_l1)

conv2 = Convolution2D(32,11,11,border_mode='same', W_regularizer=l1(wr), init='lecun_uniform')
